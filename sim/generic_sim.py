    ...
    def randomize_dynamics(self, dr_ranges):
        ...
        if "cassie" in self.__class__.__name__.lower():
            for i in range(self.num_cassie):
                ...
                for joint in self.joint_shin_list[i]:
                    self.set_jnt_stiffness(name=joint, stiffness=k_knee)

                for joint in self.joint_heel_list[i]:
                    self.set_jnt_stiffness(name=joint, stiffness=k_heel)
        # Spring stiffness
        # if "cassie" in self.__class__.__name__.lower():
        #     k_knee = np.random.uniform(2000, 3000)
