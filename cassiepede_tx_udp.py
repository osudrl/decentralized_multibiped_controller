...
def execute(policy, env, args, do_log, exec_rate=1):
    ...
    src_key_padding_mask = torch.zeros(1, env.num_cassie, dtype=torch.bool)
        ...
        while True:
            ...
            if 'cassie' in platform.node():
                ...
                if state.radio.channel[8] < 0 or (
                    ...
                    env.state_dict = {k: np.zeros((env.num_cassie, *v)) for k, v in
                                       env.observation_size.items()}
            ...
            if operation_mode == 0:
                ...
                if first or update_time > 1 / env.default_policy_rate:
                    ...
                    for k in RL_state.keys():
                        RL_state[k] = torch.tensor(RL_state[k], dtype=torch.float32, device='cpu').unsqueeze(0).unsqueeze(0)

                    with torch.no_grad():
                        # bikram, ashutosh
                        action, _ = policy(RL_state, src_key_padding_mask=src_key_padding_mask,
                                                               need_weights=False)
                        # print('action', action.shape)
                        # exit()
                        action = action.numpy().squeeze(0).squeeze(0).squeeze(0)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--transformer_hidden_dim", type=int, default=64, help="Number of hidden units in Transformer")
    ...
    # #
    # bikram
    actor = load_actor(args, device=torch.device('cpu'), model_fn=Actor_Transformer_v3)
