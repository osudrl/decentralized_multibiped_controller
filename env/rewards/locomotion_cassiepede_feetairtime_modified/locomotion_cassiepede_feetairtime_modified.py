...
def compute_rewards(self, action):
    ...
    for i in range(self.num_cassie):
        ...
        l_foot_acc = self.sim.get_body_acceleration(self.sim.feet_body_name[i][0])[:3]
        r_foot_acc = self.sim.get_body_acceleration(self.sim.feet_body_name[i][1])[:3]
        ...
        if self._height_control:
            ...
            q['height'] = -abs(robot_height - self.height_base[i])

        if self.num_cassie > 1:
            # Supplement reward for minimizing orientation error
            q['poi_orientation'] = -orientation_error
            # -1 means last item in the state history window (assumption is that all items are the same)
            q['base_yaw_poi'] = -np.abs(self.state_dict['base_yaw_poi'][i, -1].item()) / np.pi
        ...
        if zero_cmd \
                and self.num_cassie > 1 \
                and (not self.perturbation_force or (np.linalg.norm(self.force_vector) == 0)) \
                and (not self.perturbation_torque or (np.linalg.norm(self.torque_vector)) == 0):
            q['joint_forces'] = joint_forces[i]

        # q['terrain_collision'] = 0.0
        # # # Check collision only if terrain map is available
        # if self.terrain_map is not None:
        #     terrain_collision = False
        #     for b in self.sim.body_collision_list[i]:
        #         if self.sim.is_body_collision(b):
        #             terrain_collision = True
        #             break
        #
        #     if not terrain_collision:
        #         for g in self.sim.geom_collision_list[i]:
        #             if self.sim.is_geom_collision(g):
        #                 terrain_collision = True
        #                 break
        #
        #     if terrain_collision:
        #         q['terrain_collision'] = -10.0
...
def compute_done(self):
    ...
    if not self.enable_hfield:
        if np.degrees(np.abs(poi_orientation[1])) > 30.0:
            return np.ones(self.num_cassie, dtype=bool), ['Deck pitch too far off']
        if np.degrees(np.abs(poi_orientation[0])) > 30.0:
    ...
    for i in range(self.num_cassie):
        ...
        #
        #     if terrain_collision:
        #         done_reasons.append(f'Cassie {i} collision with terrain')
        #         dones.append(True)
        #         continue
        ...
        for b in self.sim.knee_walking_list[i]:
            # if self.sim.is_body_collision(b, exclude='geom_box_NC' if self.enable_hfield else None):
            if self.sim.is_body_collision(b):
                knee_collision = True
                break
        ...
        base_euler = R.from_quat(us2scipy(base_orientations[i])).as_euler('xyz')
        ...
        if self.enable_hfield:
            # Have some leeway for height bounds if hfield is enabled
            height_bounds = (height_bounds[0] - 0.3, height_bounds[1] + 0.3)

        if np.degrees(np.abs(base_euler[1])) > 30.0:
            done_reasons.append(f'Cassie {i} pitch too far off')
            dones.append(True)
        elif np.degrees(np.abs(base_euler[0])) > 30.0:
            done_reasons.append(f'Cassie {i} roll too far off')
            dones.append(True)
        # -1 means last item in the state history window (assumption is that all items are the same)
        elif np.degrees(np.abs(self.state_dict['base_yaw_poi'][i, -1].item())) > 30.0:
            done_reasons.append(f'Cassie {i} yaw too far off')
            dones.append(True)
        elif not (height_bounds[0] < robot_height < height_bounds[1]):
